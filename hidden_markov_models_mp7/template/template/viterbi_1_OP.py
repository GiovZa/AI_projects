...
def viterbi_1(train, test, get_probs=training):
    ...
    output: list of sentences, each sentence is a list of (word,tag) pairs.
            E.g., [[(word1, tag1), (word2, tag2)], [(word3, tag3), (word4, tag4)]]
    '''
    # 1. Get initial, emission, and transition probabilities from training data
    init_prob, emit_prob, trans_prob = get_probs(train)
    
    # 2. Compute the most frequent tag from training data (as done in the baseline)
    tags = defaultdict(lambda: 0)
    for sentence in train:
        for w, t in sentence:
            tags[t] += 1
    most_frequent_tag = max(tags, key=tags.get)  # Most common tag in the training data
    ...
    for sen in range(len(test)):
        ...
        # 3. Initialize log probabilities for first word in the sentence
        for t in emit_prob:
            if t in init_prob:
                log_prob[t] = log(init_prob[t])
            else:
        ...
        # 4. Forward steps to calculate log probs for sentence
        for i in range(length):
            word = sentence[i]
            
            # Check if the word is seen in training; if not, use the most common tag
            if sentence[i] not in emit_prob[most_frequent_tag]:
                sentence[i] = '<UNK>'  # Treat unseen word as unknown
            
            log_prob, predict_tag_seq = viterbi_stepforward(i, sentence[i], log_prob, predict_tag_seq, emit_prob, trans_prob)
        
        # 5. Final prediction by backtracing the best path
        best_prev_tag = max(log_prob, key=log_prob.get)
        best_tag_seq = predict_tag_seq[best_prev_tag]
...
def viterbi_1(train, test, get_probs=training):
    ...
    for sen in range(len(test)):
        ...
        for i in range(length):
            word = sentence[i]
            if word not in emit_prob[most_frequent_tag]:
                word = '<UNK>'
            
            log_prob, predict_tag_seq = viterbi_stepforward(i, word, log_prob, predict_tag_seq, emit_prob, trans_prob)
