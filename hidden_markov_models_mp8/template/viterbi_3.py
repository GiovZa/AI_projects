...
def get_probabilities(tag_counts, word_tag_counts, transition_counts, hapax_tag_type_prob):
    total_hapax = sum(sum(type_counts.values()) for type_counts in hapax_tag_type_prob.values())
    ...
    for t in tag_counts:
        for word_type in hapax_tag_type_prob[t]:
            hapax_prob = hapax_tag_type_prob[t][word_type] / total_hapax if total_hapax > 0 else alpha / len(tag_counts)
            emis_probs[t][word_type] = hapax_prob

    for prev_t, next_tags in transition_counts.items():
        total_count = sum(next_tags.values())
        for next_tag, count in next_tags.items():
            trans_probs[prev_t][next_tag] = count / total_count

    return emis_probs, trans_probs
...
def viterbi_3(train, test):
    ...
    emis_probs, trans_probs = get_probabilities(tag_counts, word_tag_counts, transition_counts, hapax_tag_type_prob)
        ...
        for w in range(1, len(s)):
            ...
            for t in all_tags:
                ...
                for prev_t in all_tags:
                    ...
                    emis_prob = emis_probs[t].get(s[w], alpha * hapax_tag_type_prob[t].get(word_type, 2 * alpha / len(hapax_tag_type_prob)))
