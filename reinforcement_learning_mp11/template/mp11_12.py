...
class Application:
    ...
    def execute(self):
        if self.args.early_check:
            ...
            # Debugging: Print gold and test adjoining wall values side-by-side
            print("\nDebugging Adjoining Wall Values:")
            for i, (gold, test) in enumerate(zip(gold_results, test_results)):
                if gold[2] != test[2] or gold[3] != test[3]:
                    print(f"Test {i + 1}:")
                    print(f"  Gold adjoining_wall_x: {gold[2]}, adjoining_wall_y: {gold[3]}")
                    print(f"  Test adjoining_wall_x: {test[2]}, adjoining_wall_y: {test[3]}")
                    print("-" * 30)
