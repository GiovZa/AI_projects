...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        # Define the possible moves (below, left, above, right)
        directions = {
            'below': (1, 0),
            'left': (0, -1),
            'above': (-1, 0),
        ...
        moves = ['below', 'left', 'above', 'right']

        for move in moves:
            d_row, d_col = directions[move]
            neighbor_row, neighbor_col = zero_row + d_row, zero_col + d_col

            # Check if neighbor position is within borders
            if 0 <= neighbor_row < 3 and 0 <= neighbor_col < 3:
                # Utilize 'import copy' advice
                updated_state = copy.deepcopy(self.state)

                # Swap the empty and neighbor tiles
                updated_state[zero_row][zero_col], updated_state[neighbor_row][neighbor_col] = updated_state[neighbor_row][neighbor_col], updated_state[zero_row][zero_col]
                
                updated_zero_loc = (neighbor_row, neighbor_col)
                nbr_states.append(EightPuzzleState(updated_state, self.goal, self.dist_from_start + 1, self.use_heuristic, updated_zero_loc))
    ...
    def compute_heuristic(self):
        ...
        # Handle goal index looping outside main loop 
        # So we don't have a 4-layer 
        flat_goal = []
        for row in self.goal:
            for tile in row:
                flat_goal.append(tile)
        ...
        for i in range(row):
            for j in range(col):
                ...
                goal_index = flat_goal.index(current_tile)
                goal_i, goal_j = divmod(goal_index, 3)
                total += manhattan((i, j), (goal_i, goal_j))
